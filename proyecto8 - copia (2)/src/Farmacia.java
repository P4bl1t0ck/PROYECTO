...
public class Farmacia {
    ...
    public void compra(int id, int cant) {
        try {
            ...
            if (medicina != null) {
                if (medicina.getStock() >= cant) {
                    double nuevoPrecio = cant * medicina.getPrecio();
                    medicina.setStock(medicina.getStock() - cant);

                    String respuesta = "Usted hizo una compra de [" + cant + "] de [" + medicina.getNombre() + "] por el precio de [$" + nuevoPrecio + "].";
                    System.out.println(respuesta);
                } else {
                    System.out.println("Error: Stock insuficiente para la compra.");
                }
            } else {
